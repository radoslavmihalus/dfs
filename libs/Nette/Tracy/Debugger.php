...
class Debugger {
    ...
    public static function log($message, $priority = self::INFO) {
        ...
        if ($message instanceof \Exception) {
            $exception = $message;
            while ($exception) {
                $tmp[] = ($exception instanceof ErrorException ? 'Fatal error: ' . $exception->getMessage() : get_class($exception) . ': ' . $exception->getMessage())
    ...
    private static function isHtmlMode() {
        return empty($_SERVER['HTTP_X_REQUESTED_WITH']) && PHP_SAPI !== 'cli' && !preg_match('#^Content-Type: (?!text/html)#im', implode("\n", headers_list()));
    }
