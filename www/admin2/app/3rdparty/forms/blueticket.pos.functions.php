...
function before_federated_item_insert_callback($row_data, $primary) {
    $tbl = $row_data->get('federated_templates.MasterTableName');

    $btdb = blueticket_forms_db::get_instance();
    $btdb->query("SHOW CREATE TABLE $tbl");

    $row = $btdb->row();

    $createStatement = "# {POSID} - ID of remote POS for template\n";
    $createStatement .= "# {DBAddress} - Address of remote POS db for template : ENGINE=FEDERATED DEFAULT CHARSET=utf8 CONNECTION='mysql://user:pass@ipaddress/dbname/tablename'\n";
    $createStatement .= $row['Create Table'] . ';';

    $row_data->set('CreateTemplate', $createStatement);
}
