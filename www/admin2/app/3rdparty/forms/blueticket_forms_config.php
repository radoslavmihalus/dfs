...
/** Configuration file; blueticket® blueticketFORMS v.1.0; 06/2014 */

class blueticket_forms_config {

    // default connection
    public static $dbname = 'rcrud'; //u147232041_rpos'; // Your database name
    ...
    // theme and language
    public static $theme = 'bootstrap'; // can be 'default', 'bootstrap', 'minimal' or your custom. Theme of blueticketFORMS visual presentation. For using bootstrap you need to load it on your page.
    public static $language = 'sk'; // sets default localization
    ...
    // scripts
    public static $load_bootstrap = false; // turn on, if you want to load bootstrap via blueticketFORMS
    public static $load_googlemap = false; // loads google map api for 'POINT' type. Turn off, if your site already uses it.
    public static $load_jquery = false; // loads jQuery, turn it off if you already have jQuery on your page. jQuery version must be at least 1.7. If your jQuery loads in the bottom of page, you must activate $manual_load and use  blueticket_forms::load_css() & blueticket_forms::load_js() on your page.
    ...
    public static $manual_load = false; // Allows you to disable blueticket_formss css and js output, but you can use blueticket_forms::load_css() & blueticket_forms::load_js() in your code manually.
    // editor
    ...
    public static $start_minimized = false; // Start all blueticketFORMS instances minimized.
    ...
    public static $upload_folder_def = '../../../uploads'; // Default uploads folder on your site, relative to blueticketFORMS folder or absolute path required. Folder is must exist.
    // features
    ...
    public static $nested_readonly_on_view = false; // turn of editing nested tables when viewing parent (can edit only when editing parent)
    ...
    public static $email_from_name = 'blueticketFORMS Data Management System'; // email from name
    ...
    // blueticket_forms folder url
    public static $scripts_url = ''; // URL to the blueticketFORMS folder, not real path, without a trailing slash, can be relative, e.g. 'some_folder/blueticket_forms' or absolute, e.g. 'http://www.your_site.com/some_folder/blueticket_forms'. If empty - will be detected automatically
    ...
    // urls (relative to $scripts_url or blueticket_forms's folder, if $scripts_url is not defined)
    ...
    public static $ajax_uri = 'blueticket_forms_ajax.php'; // main ajax file or url
    // paths (relative to blueticket_forms's folder)
    public static $themes_path = 'themes'; // php and ini files
    public static $lang_path = 'languages'; // ini files
    // external session
    public static $external_session = true; // use only when you use integration with externall session
    // loading events
    ...
    public static $autoclean_timeout = 3; // in seconds. Do not change, if not sure. blueticket_forms clears old instances in session when you reload browser tab or open new tab with blueticket_forms. In this case blueticket_forms can't work in two tabs in the same time. You can increase timeout on your risk.
    // anti XSS
    public static $auto_xss_filtering = false; // enable all blueticket_forms's POST and GET data filtering
