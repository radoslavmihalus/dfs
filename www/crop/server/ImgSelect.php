...
class ImgSelect {
    ...
    protected function upload() {
        ...
        $file = $this->handleFileUpload(
                $upload['tmp_name'], $upload['name'], isset($upload['size']) ? $upload['size'] : strlen(@$upload['tmp_name']), isset($upload['error']) ? $upload['error'] : null
        );
    ...
    protected function crop() {
        ...
        $this->resizeImage($crop->src_path, $crop->dst_path, $crop->src_x, $crop->src_y, $crop->dst_w, $crop->dst_h, $crop->src_w, $crop->src_h);
    ...
    public function upcountName($name) {
        return preg_replace_callback(
                '/(?:(?: \(([\d]+)\))?(\.[^.]+))?$/', array($this, 'upcountNameCallback'), $name, 1
        );
    }
